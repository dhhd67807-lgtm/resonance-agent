...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	private _generateChatMessagesSystemMessage = async (chatMode: ChatMode, specialToolFormat: 'openai-style' | 'anthropic-style' | 'gemini-style' | undefined) => {
		...
		console.log('[VOID DEBUG] _generateChatMessagesSystemMessage:', {
			specialToolFormat,
			includeXMLToolDefinitions,
			notSpecialToolFormat: !specialToolFormat
		})
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { providerName, modelName } = modelSelection
		const modelCapabilities = getModelCapabilities(providerName, modelName, overridesOfModel)
		...
		if (providerName === 'openAICompatible' && specialToolFormat) {
			console.log('[VOID DEBUG] Forcing XML tools for openAICompatible provider model:', modelName);
