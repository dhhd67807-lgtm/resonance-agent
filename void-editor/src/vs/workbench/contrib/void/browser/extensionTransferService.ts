	...
	if (os === 'mac') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Resonance', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, 'Library', 'Application Support', 'Resonance', 'User', 'keybindings.json'),
			}, {
	...
	if (os === 'linux') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Resonance', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.config', 'Resonance', 'User', 'keybindings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.vscode', 'extensions'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), homeDir, '.void-editor', 'extensions'),
				isExtensions: true,
	...
	if (os === 'windows') {
		...
		if (fromEditor === 'VS Code') {
			return [{
				...
				to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Resonance', 'User', 'settings.json'),
			}, {
				from: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Code', 'User', 'keybindings.json'),
				to: URI.joinPath(URI.from({ scheme: 'file' }), appdata, 'Resonance', 'User', 'keybindings.json'),
			}, {
