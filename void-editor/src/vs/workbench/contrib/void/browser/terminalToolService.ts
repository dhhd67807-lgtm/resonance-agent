...
export class TerminalToolService extends Disposable implements ITerminalToolService {
	...
	runCommand: ITerminalToolService['runCommand'] = async (command, params) => {
		...
		if (isPersistent) { // BG process
			...
			if (!terminal) throw new Error(`Terminal creation failed: Persistent terminal with ID "${persistentTerminalId}" does not exist. Please create it first using open_persistent_terminal.`);
		}
		else {
			// Use the active terminal if available, otherwise create a visible one
			const activeTerminal = this.terminalService.activeInstance;
			if (activeTerminal) {
				terminal = activeTerminal;
			} else {
				// Create a visible terminal in the panel instead of hidden
				const { cwd } = params
				terminal = await this._createTerminal({ cwd: cwd, config: undefined, hidden: false })
			}
		...
		const waitForResult = async () => {
			// Focus the terminal so user can see the command running
			this.terminalService.setActiveInstance(terminal)
			await this.terminalService.focusActiveInstance()
			...
			const waitUntilDone = new Promise<void>(resolve => {
				...
				const l = cmdCap.onCommandFinished(cmd => {
					...
					// Don't capture output - let it stay in terminal only
					l.dispose()
					resolve()
				})
				disposables.push(l)
			...
			// Don't read terminal output - let it stay in terminal only
			...
			// Return concise user-friendly messages
			let userMessage = '';
			
			if (resolveReason.type === 'done') {
				if (resolveReason.exitCode === 0) {
					if (isPersistent) {
						userMessage = `Running in background terminal. Check terminal panel for live output.`;
					} else {
						userMessage = `Command completed successfully.`;
					}
				} else {
					userMessage = `Command failed (exit code ${resolveReason.exitCode}). Check terminal for details.`;
				}
			} else if (resolveReason.type === 'timeout') {
				if (isPersistent) {
					userMessage = `Running in background terminal.`;
				} else {
					userMessage = `Command timed out after ${MAX_TERMINAL_INACTIVE_TIME}s. Check terminal for output.`;
				}
			}

			return { result: userMessage, resolveReason }
