...
const RefreshModelButton = ({ providerName }: { providerName: RefreshableProviderName }) => {
	...
	const { state } = refreshModelState[providerName] || { state: 'idle' };
...
export const ModelDump = ({ filteredProviders }: { filteredProviders?: ProviderName[] }) => {
	...
	// Use either filtered providers or all visible providers (excluding hidden ones)
	const providersToShow = filteredProviders || visibleProviderNames;
...
export const SettingsForProvider = ({ providerName, showProviderTitle, showProviderSuggestions }: { providerName: ProviderName, showProviderTitle: boolean, showProviderSuggestions: boolean }) => {
	...
	return <div>
		...
		<div className='px-0'>
			{/* settings besides models (e.g. api key) */}
			{settingNames.map((settingName, i) => {
				// Hide endpoint field for nvidiaFree provider
				if (providerName === 'nvidiaFree' && settingName === 'endpoint') {
					return null;
				}
				...
				<main className="flex-1 p-6 select-none">
					...
					<div className='max-w-3xl'>
						...
						<div className='flex flex-col gap-12'>
							...
							{/* Free section (formerly Local Providers) */}
							<div className={shouldShowTab('localProviders') ? `` : 'hidden'}>
								<ErrorBoundary>
									<h2 className={`text-3xl mb-2`}>Free</h2>
									<h3 className={`text-void-fg-3 mb-2`}>{`Access free AI models powered by NVIDIA.`}</h3>

									<VoidProviderSettings providerNames={['nvidiaFree']} />
								</ErrorBoundary>
							</div>
