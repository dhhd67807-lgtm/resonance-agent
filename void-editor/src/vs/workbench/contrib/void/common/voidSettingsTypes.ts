...
// Providers that should be hidden from the UI (internal/premium only)
export const hiddenProviderNames = [] satisfies ProviderName[]

// Visible providers (filter out hidden ones)
...
export const subTextMdOfProviderName = (providerName: ProviderName): string => {
	...
	if (providerName === 'nvidiaFree') return 'Get your free [NVIDIA API Key here](https://build.nvidia.com/explore/discover).'
...
const modelInfoOfDefaultModelNames = (defaultModelNames: string[]): { models: VoidStatefulModelInfo[] } => {
	return {
		models: defaultModelNames.map((modelName, i) => ({
			modelName,
			type: 'default',
			isHidden: false, // Show NVIDIA model by default
		}))
	}
}
...
export const defaultSettingsOfProvider: SettingsOfProvider = {
	...
	nvidiaFree: {
		...defaultCustomSettings,
		...defaultProviderSettings.nvidiaFree,
		...modelInfoOfDefaultModelNames(defaultModelsOfProvider.nvidiaFree),
		_didFillInProviderSettings: undefined,
	},
	gemini: {
		...defaultCustomSettings,
...
export type GlobalSettings = {
	...
	isWelcomeScreenComplete: boolean;
