...
const defaultState = () => {
	const d: VoidSettingsState = {
		...
		modelSelectionOfFeature: { 
			'Chat': { providerName: 'openAICompatible', modelName: 'claude-3.5-sonnet' },
			'Ctrl+K': { providerName: 'openAICompatible', modelName: 'claude-3.5-sonnet' },
			'Autocomplete': { providerName: 'openAICompatible', modelName: 'claude-3.5-sonnet' },
			'Apply': { providerName: 'openAICompatible', modelName: 'claude-3.5-sonnet' },
			'SCM': { providerName: 'openAICompatible', modelName: 'claude-3.5-sonnet' }
		},
	...
	async readAndInitializeState() {
		...
		try {
			...
			for (const providerName of providerNames) {
				...
				// ensure nvidiaFree has pre-filled values
				if (providerName === 'nvidiaFree') {
					if (!readS.settingsOfProvider[providerName].endpoint) {
						readS.settingsOfProvider[providerName].endpoint = defaultProviderSettings.nvidiaFree.endpoint
					}
					if (!readS.settingsOfProvider[providerName].apiKey) {
						readS.settingsOfProvider[providerName].apiKey = defaultProviderSettings.nvidiaFree.apiKey
					}
				}
			}
		}
