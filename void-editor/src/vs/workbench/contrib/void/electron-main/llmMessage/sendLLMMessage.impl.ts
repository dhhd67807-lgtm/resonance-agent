	...
	if (providerName === 'openAI') {
		...
		return new OpenAI({
			...
			defaultHeaders: {
				...
				'X-Title': 'Resonance', // Optional. Shows in rankings on openrouter.ai.
			},
			...commonPayloadOpts,
		})
	}
...
const _sendOpenAICompatibleChat = async ({ messages, onText, onFinalMessage, onError, settingsOfProvider, modelSelectionOptions, modelName: modelName_, _setAborter, providerName, chatMode, separateSystemMessage, overridesOfModel, mcpTools }: SendChatParams_Internal) => {
	...
	// CRITICAL FIX: OpenAI-compatible providers should use XML tools, not native format
	// Native tool formats (openai-style, anthropic-style) only work with official APIs
	if (providerName === 'openAICompatible' && specialToolFormat) {
		console.log('[VOID DEBUG] ELECTRON-MAIN: Forcing XML tools for openAICompatible provider model:', modelName_, 'was:', specialToolFormat);
	...
	console.log('[VOID DEBUG] Tool calling setup:', {
		chatMode,
		specialToolFormat: specialToolFormat ?? 'XML (undefined)',
		hasPotentialTools: !!potentialTools,
		numTools: potentialTools?.length ?? 0,
		willSendNativeTools: !!(potentialTools && specialToolFormat === 'openai-style'),
		willUseXMLTools: !specialToolFormat
	})
	const nativeToolsObj = potentialTools && specialToolFormat === 'openai-style' ?
		{ tools: potentialTools, tool_choice: 'auto' as const } as const
		: {}
	...
	if (!specialToolFormat) {
		console.log('[VOID DEBUG] Applying XML extraction wrapper for OpenAI, chatMode:', chatMode);
		const { newOnText, newOnFinalMessage } = extractXMLToolsWrapper(onText, onFinalMessage, chatMode, mcpTools)
		onText = newOnText
		onFinalMessage = newOnFinalMessage
		console.log('[VOID DEBUG] XML extraction wrapper applied for OpenAI');
	} else {
		console.log('[VOID DEBUG] Skipping XML extraction for OpenAI, specialToolFormat:', specialToolFormat);
	}
	...
	retryWithExponentialBackoff(
		...
		.then(async response => {
			...
			if (!fullTextSoFar && !fullReasoningSoFar && !toolName) {
				onError({ message: 'Resonance: Response from model was empty.', fullError: null })
			}
			else {
				let toolCall = rawToolCallObjOfParamsStr(toolName, toolParamsStr, toolId)
				
				// If no tool call was made, try to enforce one
				if (!toolCall && chatMode === 'agent') {
					toolCall = enforceToolCall(fullTextSoFar, chatMode);
					if (toolCall) {
						console.log('[VOID DEBUG] Tool call enforcer created tool call from text response');
					}
				}
				
				console.log('[VOID DEBUG] LLM response completed:', {
					hasText: !!fullTextSoFar,
