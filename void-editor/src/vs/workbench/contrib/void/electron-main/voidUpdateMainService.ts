...
export class VoidMainUpdateService extends Disposable implements IVoidUpdateService {
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<VoidCheckUpdateRespose> {
		try {
			...
			const response = await fetch('https://api.github.com/repos/dhhd67807-lgtm/resonance-agent/releases/latest');
			...
			if (explicit) {
				if (response.ok) {
					if (!isUpToDate) {
						message = 'A new version of Resonance is available! Please reinstall (auto-updates are disabled on this OS) - it only takes a second!'
						action = 'reinstall'
					}
					else {
						message = 'Resonance is up-to-date!'
					}
				}
				else {
